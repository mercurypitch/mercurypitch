...
The first supported mechanic is a comfortable held note. The UI discovers a stable comfortable pitch, plays its reference, waits for the audio-clock release and quiet gap, then enables scoring. The default hold is 1.2 s within 150 cents with confidence at least 0.5. A 150 ms breath grace precedes gradual decay. These are game tuning values, not clinical thresholds. The safe interaction radius is currently 1.1 m, with grounded whole-body support on a deck; the 0.46 m gold approach ring marks the centre to approach rather than a strict foot-placement boundary. The next required marker follows the exit dependency chain; optional side exhibits do not seal the exit.

`PitchObservation` carries a stable sequence, capture-clock seconds and the original capture time mapped to `performance.now()`. `F0Stream.subscribeCaptured` publishes every raw accepted worker result, including silence, independently of render polling. Duplicate, out-of-order, stale, weak and out-of-target frames cannot manufacture hold time. Success uses consecutive valid capture duration; absolute-clock decay prevents double-counting intervals between audio callbacks and slow render frames. Ordered lower/higher pairs reuse this hold judge through the shared challenge session; comfortable pair calibration and per-step reference replay are implemented in Twin Galleries. Vibrato remains a future mechanic requiring its own judge and tests.
