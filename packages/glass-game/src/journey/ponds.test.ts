...
function conservatoryPlinthRadius(): number {
  ...
  const node = document.nodes.find(
    (candidate: { name: string }) =>
      candidate.name === 'map_conservatory_geometry',
  )
  ...
  const offset =
    jsonEnd + 8 + (view.byteOffset ?? 0) + (accessor.byteOffset ?? 0)
  let radius = 0
  for (let index = 0; index < accessor.count; index++) {
    ...
    radius = Math.max(
      radius,
      Math.hypot(bytes.readFloatLE(point), bytes.readFloatLE(point + 8)),
    )
  }
...
describe('museum water garden placement', () => {
  it('keeps the complete pond coping clear of the actual Conservatory base', () => {
    ...
    const borderRadius = Math.max(
      0.023,
      0.02 * Math.max(source.width, source.length),
    )
    ...
    for (let index = 0; index < 128; index++) {
      ...
      const worldX =
        source.position[0] +
        x * Math.cos(spillway.yaw) +
        z * Math.sin(spillway.yaw)
      const worldZ =
        source.position[2] -
        x * Math.sin(spillway.yaw) +
        z * Math.cos(spillway.yaw)
      clearance = Math.min(
        clearance,
        Math.hypot(
          worldX - stage.architecturePosition[0],
          worldZ - stage.architecturePosition[2],
        ) -
          baseRadius -
          borderRadius,
      )
    ...
    expect(
      clearance,
      'Pond border needs visible space beside the imported plinth',
