...
# (run from your feature branch; note the slash in origin/main)
...
- **NEVER** use `git reset --hard` for rebasing. Always use `git rebase origin/BRANCHNAME`
  (`git rebase origin main` without the slash checks out `main` instead of rebasing your branch)
- `--force-with-lease` after a rebase is fine; plain `--force` is never allowed
