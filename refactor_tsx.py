...
module_css_files = [f for f in os.listdir(components_dir) if f.endswith(".module.css")]
...
def process_file(filepath, module_name):
    ...
    css_filepath = os.path.join(components_dir, f"{module_name}.module.css")
    ...
    if "import styles from" not in content and f"import styles from './{module_name}.module.css'" not in content:
        # Find last import to place it after
        last_import_idx = content.rfind("import ")
        if last_import_idx != -1:
            end_of_line = content.find("\n", last_import_idx)
            content = content[:end_of_line+1] + f"import styles from './{module_name}.module.css'\n" + content[end_of_line+1:]
        else:
            content = f"import styles from './{module_name}.module.css'\n" + content
...
# Map components to their TSX files
tsx_files = [f for f in os.listdir(components_dir) if f.endswith(".tsx")]
for tsx_file in tsx_files:
    module_name = tsx_file.replace(".tsx", "")
    if process_file(os.path.join(components_dir, tsx_file), module_name):
        print(f"Refactored {tsx_file}")
