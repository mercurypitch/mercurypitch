...
`src/features/guitar-night/useGuitarNightScoreRoomController.ts` — Guitar Night
count-in and playback-click preferences
**Tests:** `src/e2e/metronome.spec.ts` (`REQ-MET-001..019`);
`src/features/guitar-night/useGuitarNightScoreRoomController.test.tsx` and
`src/e2e/guitar-night-room-controls.spec.ts` (`REQ-MET-020`)
...
### REQ-MET-020 — Guitar Night remembers count-in and click independently

**WHEN** the user changes Guitar Night's count-in or playback-click setting,
the shared score-room controller **shall** persist that preference on this
device and restore it across page reloads, song changes, and recorded-melody
practice sessions. Zero count-in beats and a disabled click **shall** remain
valid preferences. Restoring settings **shall** not activate audio or start
playback. Assessment-only click suppression **shall** not overwrite the
user's saved playback-click choice.
