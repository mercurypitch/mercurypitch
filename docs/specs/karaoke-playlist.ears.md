...
- **IMPORT-4** — WHEN an exported karaoke playlist references a full-band session, the archive shall include every stored Vocal, Instrumental, Drums, Bass, Guitar, Piano and Other stem, and re-import shall restore every included stem with its supported audio format preserved.
- **IMPORT-5** — WHEN a karaoke archive is imported, the system shall validate its manifest and session data, accept only known stem identifiers, prevent archive paths or filenames from escaping their session, and reject entries that exceed the supported import limits.
- **IMPORT-6** — IF durable storage of any required session data fails during import, THEN the system shall report the failed session and remove its partially imported session, stems and auxiliary data rather than presenting an incomplete song as successfully restored.
- **IMPORT-7** — WHEN a group-backed karaoke playlist is exported, the system shall derive membership from each session's canonical `groupId`, rather than a potentially stale denormalized group member index.
- **IMPORT-8** — IF restoring a karaoke manifest fails after creating any groups or playlists, THEN the system shall remove those newly created records and every session imported for that manifest rather than leaving a partial karaoke set.
