...
| B2  | **Earpiece routing**: enabling mic reroutes playback to the quiet earpiece                                                                                                                   | Play melody, toggle mic, listen to output level/route                                       | ~~`@capgo/capacitor-plugin-audiosession`~~ **DONE 2026-09-01, no plugin**: `ios/App/App/AudioSession.swift` sets `playAndRecord` + `.defaultToSpeaker` and re-asserts `overrideOutputAudioPort(.speaker)` on every route change. This row shipped un-actioned in builds up to 160 and is why iOS had no audible sound at all. |
