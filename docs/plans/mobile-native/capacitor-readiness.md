...
| B10 | **Packaged media reads report status 0**: Capacitor's iOS `WebViewAssetHandler` answers a non-Range GET for a media extension (m4a, mp3, wav, …) with a bare `URLResponse`, so `fetch()` gives `ok: false, status: 0` and the whole body; Android answers 200. Media elements send Range requests and are unaffected                                                                                                        | Tap the Sing door, then Developer → Audio: `status 0 · ok false`, decoded, 1 started        | **FIX PENDING DEVICE CONFIRMATION 2026-09-25**: every packaged media read goes through `@irchiinnuss/mobile-runtime/asset-fetch` (a 2xx, or status 0 with a non-empty body), never `if (!response.ok) throw`. Beside Cue found it 4 Sep; the alley ambients and the exercise waveform were silent in TestFlight 0.5.0 (336). `probe-bundle.mjs` emulates the shape locally. MISTAKES.md, audio section                                                                                                         |
