...
| 2026-08-28 | P2.12 — swallowed keys say so: `EarStage` tracks every keydown in capture and, on a keyup of a registered key with no keydown behind it, shows the one-time note (`ear-stage-swallowed`); a whole digit clears it.                                                                                                                                                                                                    |
| 2026-08-28 | P2.13 — first/second lamps: `StagePad` takes `lamp` (`data-lamp`); The Pull (via `padLamp` on `IdentificationDrillView`), Beat Hunt and Weight light the pad whose stimulus is sounding.                                                                                                                                                                                                                              |
| 2026-08-28 | P2.14 — Span's idle line names the staircase's start length (`run.level()` at idle).                                                                                                                                                                                                                                                                                                                                  |
