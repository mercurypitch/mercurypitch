...
2. **Visual quality on a phone.** The specs prove the layout does not break;
   nobody has _looked_ at it. The 9:16 sky plate and the `hover: hover` gating
...
2. **The best asset in the building is behind a 90px pill.** `/mirror` (the
   ...
   Mirror" pill is a _full-page navigation out of the app_. Our strongest first
...
The brand is _Liquid Precision_ — quicksilver, obsidian, a spectrum meniscus
...
| --- | --------------- | --------------------------------------------------------------------------------------------------------------------------- | ----- | ---- |
| 1   | **Sky**         | Dark field, dead constellation, Merc drifts in. _"Your voice, made visible."_ One button: **Sing one note**                 | spine | ~5s  |
...
| 3   | **Fork**        | _"That's a G3. Want the whole map?"_ → **Map my whole voice** or **Take me in**                                             | spine | —    |
| 4   | **Voiceprint**  | Glide → Hold → Match 5 (the existing Mirror tasks, restaged). Range draws as a constellation arc                            | fork  | ~90s |
| 5   | **Twin**        | _"Your range overlaps Freddie Mercury's."_ Portrait, numbers, share card                                                    | fork  | ~10s |
...
first-stop logic against the _latest_ practice data, so it stays useful at week
...
- **The growth story.** `computeDelta` (`metrics.ts`) already computes
  ...
  baseline. With an account it becomes a real timeline: _"+3 semitones since
  March."_ This is the retention hook, and it is mostly already written.
...
2. **First streak day 2** — _"Two days. Don't lose the streak."_ In the streak
   card on `HomePage.tsx`.
3. **First challenge score** — _"Ranked #14 this week. Sign in to hold your
   place."_
...
1. _Liquid mercury nebula_ filling the frame — rejected, far too much mercury.
...
surface would also be circular. Instead the Map _offers_ each room's own tour,
...
- **Art:** obsidian sky plates (16:9 + a true 9:16 recomposition) shipped. The
  ...
  _reuse_, not new art. The ambient loop stays optional.
