...
- [x] Engine picker — Browser / Whisper / Moonshine — in Settings AND on the mic pill itself (cog on hover), with a speech-to-text latency readout in the pill. Whisper's flat ~400-500 ms chip is its fixed 30 s decode window; Moonshine's compute scales with utterance length, hence ~100-150 ms on short commands
...
- [x] `microphone on` / `microphone off` / `toggle mic` (the M shortcut; does not affect voice control's own listener) — shipped with the karaoke adapter slice
- [ ] `record` / `stop recording` (recording controller)

### Voice help

- [x] `what can i say` / `voice help` / `voice commands` — the command center: one column, sticky section headers, filter box, generated from the registry so it can never go stale. Every row is a button that RUNS its command (for when you need to stay quiet). Also opens from `Shift+V` and the mic pill's menu (`?` too on the standalone Night pages — in the main app `?` stays with the keyboard-shortcut help); Escape or a click closes it
...
- [x] M1 — trigger and the listening stage: pitch trail, top-3 with confidence breakdown, cancel by `cancel` / `stop listening` / Escape / click, manual pick by tap or `sing number one`. Say it any of these ways:
  - `mercury sing` / `shazam sing` — the brand phrases, via the grammar's brand-phrase retry
  - `what song is this` / `what song am i singing` / `what am i singing` — plain English, nothing to remember
  - `find my song` / `find this song` / `name this song` / `name that song` / `identify this song` / `match my song`
  - Recognizers that write the surname `Singh` for `sing` are repaired in normalization, so the brand phrases survive the correction
...
- [x] M4a — the wheel and the stage: four closest matches on a rotary wheel around a photographic Mercury core (curved titles along the lattice, lock-in ring while the policy arms), named "listening for" roster before the first match, the recognizer's transcript shown as a `heard` line (commands only — M2b will feed it to the match), and a ranking freeze while you SPEAK so "sing number one" can never open a reshuffled slot
- [ ] M4b — remaining polish: confidence sparklines, "not that one" rejection, recent-practice bias, in-app variant (Karaoke tab instead of the Night page, behind a setting)
