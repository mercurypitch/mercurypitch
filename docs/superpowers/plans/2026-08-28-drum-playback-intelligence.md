...
- **PR E — Pattern library** (DONE in this branch):
  - [x] `src/features/drum-night/patterns/drum-pattern.ts` — a per-voice sixteenth-grid string format (`X` accent / `x` normal / `o` ghost / `-` rest, `|` decorative), total parsing that reports bad symbols instead of throwing, GM write-back that round-trips `drumVoiceForMidi`, and projection to both a `DrumSessionDocument` and the groove editor's step model.
  - [x] `drum-pattern-library.ts` — 16 grooves, at least three per humanize style (rock/funk/jazz/latin/electronic), each with tempo, tempo window, description and `provenance`.
  - [x] `groove-editor.ts` `load-pattern` command — atomic whole-grid replacement as editor-origin hits, fully validated before a single hit is built, one undo entry. Deliberately NOT a `sourceDocument` swap: `drum-project.ts` rebuilds its base from `createFirstPocketGroove(variantId)` and validates every `source` hit against that catalogue, so a swapped source would fail restore. Editor-origin hits already round-trip.
  - [x] `DrumPatternPicker.tsx` + module CSS — style rail, per-groove detail, and a two-step confirm because loading is destructive. Lazy-loaded from `DrumNightApp`; the `patterns/index.ts` barrel stays data-only so the component never lands in the shell chunk.
  - [x] Tests: 85 in `patterns/` (format, catalog drift gate over every entry, picker behaviour), 3 reducer cases in `groove-editor.test.ts`, 2 room-level cases in `DrumNightApp.test.tsx`.
  - Grids are straight by design — swing, micro-timing and ghost shaping stay with PR B's feel engine, which is why the jazz grooves are notated on plain eighths.
  - [ ] Not seeded from GMD. Shipping GMD-derived note grids is permitted under CC-BY 4.0 but moves the app's bundled content from "no obligation" to "attribution required", unlike PR B's statistics-only posture. The `provenance` field exists so corpus- or user-seeded patterns can join later without a schema change — the user's own MIDI/drum-pattern collection is the intended next input (see dotfiles TASKS.md).
