...
|---|---|---|---|
| `guitar-night` | [guitar-night-voice-commands.ts](../../src/features/guitar-night/guitar-night-voice-commands.ts) | 17.8k | Guitar Night voice commands — the room's spoken transport Registered by GuitarNightRoom for the room's lifetime, over the SAME GuitarBack... |
...
| `guitar` | [guitar-backing-transport.ts](../../src/features/guitar/backing/guitar-backing-transport.ts) | 3.2k | Guitar backing transport keeps separated stems on one route-owned Web Audio clock. |
...
| `home` | [DestinationGallery.tsx](../../src/features/home/DestinationGallery.tsx) | 900 | DestinationGallery — the Home page's card grid `HOME_DESTINATIONS` is the content; the component is the renderer. |
...
| `mic-feedback` | [MicLatencyWizard.tsx](../../src/features/mic-feedback/MicLatencyWizard.tsx) | 700 | MicLatencyWizard — measure the speaker-to-microphone round trip Plays a short click track through the speakers and records what comes bac... |
...
| Module | Entry point | LOC | What it is |
|---|---|---|---|
| `db-worker` | [index.ts](../../workers/db-worker/src/index.ts) | 16.5k | ── MercuryPitch DB Worker Generic CRUD REST API over Cloudflare D1, matching the contract of the frontend ServerAdapter (src/db/adapters/... |
