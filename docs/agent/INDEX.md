...
| `stem-mixer` | [useStemMixerLyricsController.ts](../../src/features/stem-mixer/useStemMixerLyricsController.ts) | 8.5k | StemMixer Lyrics Controller — lyrics/LRC gen/blocks state + actions |
...
| `path` | [path-content.ts](../../src/features/path/path-content.ts) | 1.4k | The Ascent — guided-path content (data, not code) One path = an ordered list of themed weeks (the celestial orbs). |
| `challenges` | [AdminWeeklyPage.tsx](../../src/features/challenges/AdminWeeklyPage.tsx) | 1.4k | AdminWeeklyPage — owner-only weekly-challenge authoring (#/admin/weekly) Unlocks with the X-Admin-Key (stored locally), lists every row,... |
...
| `home` | [DestinationGallery.tsx](../../src/features/home/DestinationGallery.tsx) | 584 | DestinationGallery — the Home page's card grid `HOME_DESTINATIONS` is the content; the component is the renderer. |
...
| `keyboard` | [useKeyboardShortcuts.ts](../../src/features/keyboard/useKeyboardShortcuts.ts) | 319 | useKeyboardShortcuts — global hotkeys, mounted once by App One document-level keydown listener for the whole app. |
...
| `tabs` | [constants.ts](../../src/features/tabs/constants.ts) | 230 | ── Tab ID constants Use these everywhere instead of raw strings. |
| `routing` | [useHashRouter.ts](../../src/features/routing/useHashRouter.ts) | 225 | useHashRouter — binds the URL hash to app state, both directions The app has no file-system router. |
| `practice` | [usePracticeController.ts](../../src/features/practice/usePracticeController.ts) | 222 | usePracticeController — mic capture and scoring for the Singing tab Owns one mic lease for the duration of the practice run. |
...
| [runpod-bridge.ts](../../src/lib/runpod-bridge.ts) | 633 | RunPod bridge — HTTP request/response handling Turns the app's /api/uvr/* requests into RunPod job calls and back into the responses the... |
...
| [uvr-api.ts](../../src/lib/uvr-api.ts) | 559 | UVR API Client - Frontend Integration |
| [practice-engine.ts](../../src/lib/practice-engine.ts) | 555 | Practice Engine — Mic, pitch detection, accuracy scoring |
| [uvr-processing-pipeline.ts](../../src/lib/uvr-processing-pipeline.ts) | 553 | UVR Processing Pipeline — Unified abstraction over: • Server mode → upload → poll /status → download stems • Local mode → VocalSeparator... |
...
| [runpod.ts](../../src/lib/runpod.ts) | 442 | RunPod bridge — translate the app's /api/uvr/* contract to/from RunPod's serverless job API. |
...
| [uvr-store.ts](../../src/stores/uvr-store.ts) | 1.4k | UVR Store — stem separation: settings, job status, and session records Covers both processing modes: `local` (ONNX in-browser, WebGPU whe... |
...
| [ui-store.ts](../../src/stores/ui-store.ts) | 267 | UI Store — active tab, modal/library visibility, focus mode, first-run flags `setActiveTab` is the app's navigation primitive; `onTabTran... |
| [practice-session-store.ts](../../src/stores/practice-session-store.ts) | 254 | Practice Session Store — the multi-item guided practice run A session is an ordered list of SessionItems, each repeated N times. |
...
| [exercise-history-store.ts](../../src/stores/exercise-history-store.ts) | 95 | Exercise History Store — completed-run log and per-exercise stats `recordExerciseResult` is the single funnel every exercise calls on fin... |
...
| [usage-store.ts](../../src/stores/usage-store.ts) | 47 | Usage store — lightweight cumulative "has really used the app" tracking Persists two coarse signals across sessions: - usageMs: foregroun... |
...
| [HomePage.tsx](../../src/pages/HomePage.tsx) | 337 | HomePage — the "today" landing surface One obvious next step: your streak (with forgiveness), today's generated 5–15 min session, this we... |
...
| [ExercisesPage.tsx](../../src/pages/ExercisesPage.tsx) | 198 | Exercise selection state lives in AppShell (also set by share/deep-link and pending-drill flows), so it is threaded in rather than owned... |
| [AnalysisPage.tsx](../../src/pages/AnalysisPage.tsx) | 120 | Analysis is a focused UVR voiceprint on phones. |
...
| Module | Entry point | LOC | What it is |
|---|---|---|---|
| `db-worker` | [index.ts](../../workers/db-worker/src/index.ts) | 4.3k | ── MercuryPitch DB Worker Generic CRUD REST API over Cloudflare D1, matching the contract of the frontend ServerAdapter (src/db/adapters/... |
...
| File | LOC |
|---|---|
| [src/components/StemMixer.tsx](../../src/components/StemMixer.tsx) | 6.3k |
| [src/lib/piano-roll.ts](../../src/lib/piano-roll.ts) | 5.1k |
| [src/App.tsx](../../src/App.tsx) | 3.3k |
| [src/components/VocalAnalysis.tsx](../../src/components/VocalAnalysis.tsx) | 3.1k |
| [src/features/stem-mixer/useStemMixerLyricsController.ts](../../src/features/stem-mixer/useStemMixerLyricsController.ts) | 3.0k |
| [src/components/UvrPanel.tsx](../../src/components/UvrPanel.tsx) | 2.6k |
...
| [src/stores/uvr-store.ts](../../src/stores/uvr-store.ts) | 1.4k |
| [src/components/LibraryModal.tsx](../../src/components/LibraryModal.tsx) | 1.4k |
| [workers/db-worker/src/index.ts](../../workers/db-worker/src/index.ts) | 1.4k |
...
| `pnpm check:syntax` | `run-s typecheck lint fmt docs:index:check` |
