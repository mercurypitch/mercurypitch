...
| Module | Entry point | LOC | What it is |
|---|---|---|---|
| `jam` | [jam-catalog.ts](../../src/lib/jam/jam-catalog.ts) | 4.3k | ── Jam catalogue Turns the app's practice content into something a jam room can run. |
...
| [jam-store.ts](../../src/stores/jam-store.ts) | 1.4k | ── Jam store Reactive state management for P2P jam sessions. |
...
| Module | Entry point | LOC | What it is |
|---|---|---|---|
| `db-worker` | [index.ts](../../workers/db-worker/src/index.ts) | 8.2k | ── MercuryPitch DB Worker Generic CRUD REST API over Cloudflare D1, matching the contract of the frontend ServerAdapter (src/db/adapters/... |
...
| [src/stores/jam-store.ts](../../src/stores/jam-store.ts) | 1.4k |
