...
The rest of the app deliberately does not. `src/features/practice-intelligence/practice-activity.ts:31` defines `localDayKey` with the comment: "toISOString() would bucket a 23:30 run into tomorrow for anyone east of UTC, and yesterday's late practice into the wrong square" — and the streak calendar renders from it. So the calendar and the streak counter can disagree about which days were practiced.
...
- East of UTC (e.g. UTC+9/+13): the UTC date flips at 09:00 / 13:00 local. Practicing Monday 08:00 and Tuesday 10:00 local gives UTC keys two days apart → `gap = 2`, `missedDays = 1` → a freeze is spent, or with no freezes the streak resets to 1.
- West of UTC (e.g. UTC-7): the UTC date flips at 17:00 local. Practicing Monday 18:00 and Tuesday 09:00 local gives the same UTC key → `gap = 0`, so `advanceStreak` returns unchanged, `recordPathPracticeDay` sees `days.includes(date)` and returns, and Tuesday's practice lights no ring segment and advances no streak.

**Failure scenario.** A singer in Tokyo (UTC+9) practices every morning at 08:00 local. Each session's UTC date is the _previous_ calendar day, so Monday 08:00 → Sunday UTC and Tuesday 08:00 → Monday UTC; that part is consistent. But the moment they practice once in the afternoon — Tuesday 08:00 (UTC Monday) then Wednesday 15:00 (UTC Wednesday) — `daysBetween` reports 2, `missedDays = 1`, and a freeze is silently consumed; a singer with zero freezes has their 40-day streak reset to 1 despite practicing on consecutive days. Meanwhile the StreakCalendar, which uses `localDayKey`, shows both days lit — so the UI contradicts itself.
...
**Also now a registered-account feature, on both sides of every row.** Friend _codes_ were already gated; `request` and `accept` were not, so an anonymous identity — a device id in one browser's localStorage, with no way to sign in anywhere else — could send an ask and be sent one. Neither works out: they cannot answer from a second device, cannot take an ask back after clearing the browser, and the friendship dies with the profile leaving a dead name in somebody else's list. `accountRequired` in `friends.ts` now gates `code`, `redeem`, `request`, `accept` and the requests list with a 403; `request` additionally refuses an anonymous _target_ (a row nobody could ever answer), and `accept` re-checks the requester so a pending row predating the rule cannot be turned into a friendship. `remove` is deliberately exempt — needing an account to leave a row you needed none to enter would strand exactly the people the rule protects — and rows already `'accepted'` are left standing rather than deleted under anyone. The client mirrors it in `src/db/services/follow-service.ts` (same sentence, no round trip) and offers the account instead of the refusal in `CommunityLeaderboard.tsx`.
