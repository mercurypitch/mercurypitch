//  A SECOND COPY of apps/beside-cue/ios/App/App/AudioSession.swift, and a
//  deliberate one. The shared form -- a local Swift package both apps depend
//  on -- needs Xcode-level validation that only the macOS CI job provides,
//  and the P1 gate ("sound comes out of the speaker with the Ring/Silent
//  switch on") could not wait for it. Extracting it into that package is
//  tracked as debt in the native V1-1 plan. Until then, edit both or neither.
//
