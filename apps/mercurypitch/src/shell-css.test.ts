// The native shell's stylesheet (shell/shell.css): the dock's side padding,
// against the insets it answers to. Here rather than beside it, because
// src/shell/ runs under jsdom, which gives no file URL to read it from.
//
// A four-value shorthand runs top, right, bottom, left, and the dock had its
// two side insets the wrong way round: its right padding followed the LEFT
// inset. On an iPhone on its side the two insets are equal, so nothing
// showed; on a phone whose cutout is on one side only, the dock padded away
// from the cutout, and its controls could run under it.
