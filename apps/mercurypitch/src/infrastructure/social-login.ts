...
// variables, documented in `.env.example`, empty until the owner creates them.
// Empty is safe: the key is simply omitted, and the provider reports itself
// unavailable rather than failing halfway through a sheet.
//
    ...
    async initialize() {
      await SocialLogin.initialize({
        ...
        apple: {
          ...
          // Empty string, per the plugin's own note: anything else makes iOS
          // attempt a redirect it has nowhere to land.
          redirectUrl: '',
        },
      })
    },
