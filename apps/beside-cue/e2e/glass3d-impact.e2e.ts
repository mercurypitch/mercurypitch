...
/** The sampled time crosses a process boundary, where a software-rendered
 * frame can consume more than 100 ms. Aim an hour ahead, beyond any test's
 * bounded lifetime. pauseAt skips elapsed callbacks before interaction;
 * this does not wait an hour or advance the break being measured. */
const holdClock = async (page: Page): Promise<void> => {
...
test('a break holds, shakes, drops the pixel ratio for its burst and taps the hand in order', async ({
  ...
  // Let the first haptics-port import settle at this frame's time before
  // advancing beyond the intentional stale-tap dispatch deadline.
  await expect
...
test('under reduced motion the break keeps its taps and loses its motion', async ({
  ...
  // Reduced motion keeps the same immediate crack tap, before any tails.
  await expect
    .poll(async () => (await probe(page)).felt, { timeout: 5_000 })
    .toEqual([35])
...
test('the Sorting Line taps the hand when he drops through a grate', async ({
  ...
  // As in the Cabinet, measure dispatch in game time. A software-rendered
  // frame can otherwise delay the first port import beyond the intentional
  // 100 ms stale-tap cutoff, turning this timely-tap test into a stalled tap.
  await holdClock(page)
  await page.evaluate(() =>
    (
