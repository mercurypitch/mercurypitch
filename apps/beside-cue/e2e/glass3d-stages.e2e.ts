...
// state on DEV hooks (`__w3`, `__w3h`, `__w3c`, `__w3l`, `__w3s`), which is what
// these read.
...
for (const world of WORLDS.filter(
  (w) => w.hook === '__w3h' || w.hook === '__w3s',
)) {
  test(`${world.hook}: a stage left alone calms, and a key wakes it on the next frame`, async ({
    page,
  }) => {
    ...
    const a = (await perfOf(page, world.hook))!
    expect(a.calm).toBe(true)

    await page.clock.runFor(1_000)
    const b = (await perfOf(page, world.hook))!
