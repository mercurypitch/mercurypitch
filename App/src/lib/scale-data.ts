...
export function freqToNote(freq: number): MelodyNote & { cents: number; midi: number } {
...
export function buildMajorScale(keyName: string, octave: number): ScaleDegree[] {
...
export function buildSampleMelody(keyName: string, octave: number): MelodyItem[] {
...
export function melodyNoteAtBeat(melody: MelodyItem[], beat: number): MelodyItem | null {
  for (const item of melody) {
...
export function melodyMidiRange(melody: MelodyItem[]): { min: number; max: number } {
