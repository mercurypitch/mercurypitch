...
import { melodyStore, setMelody } from '@/stores/melody-store';
...
describe('Preset Save and Load', () => {
  ...
  describe('savePreset', () => {
    ...
    it('saves effectType and linkedTo', () => {
      const data: PresetData = {
        notes: [
          { midi: 60, startBeat: 0, duration: 1, effectType: 'vibrato', linkedTo: [1] },
          { midi: 64, startBeat: 1, duration: 1 },
        ],
        totalBeats: 2,
        bpm: 120,
    ...
    it('overwrites an existing preset with the same name', () => {
      ...
      const data2: PresetData = {
        notes: [{ midi: 72, startBeat: 0, duration: 2 }, { midi: 74, startBeat: 2, duration: 1 }],
        totalBeats: 3,
        bpm: 80,
        scale: [],
      };
  ...
  describe('getPresetNames', () => {
    it('returns sorted list of preset names', () => {
      savePreset('Zebra Preset', { notes: [], totalBeats: 1, bpm: 120, scale: [] });
      savePreset('Apple Preset', { notes: [], totalBeats: 1, bpm: 120, scale: [] });
      savePreset('Melon Preset', { notes: [], totalBeats: 1, bpm: 120, scale: [] });
  ...
  describe('deletePreset', () => {
    it('removes a preset from localStorage', () => {
      savePreset('To Delete', { notes: [{ midi: 60, startBeat: 0, duration: 1 }], totalBeats: 1, bpm: 120, scale: [] });
