...
describe('Settings — init and defaults', () => {
  ...
  it('loads from localStorage if present', () => {
    ...
    expect(s.minConfidence).toBe(0.60);
...
describe('Settings — setDetectionThreshold', () => {
  ...
  it('clamps to maximum 0.20', () => {
    setDetectionThreshold(0.30);
    expect(appStore.settings().detectionThreshold).toBe(0.20);
  });
...
describe('Settings — setMinConfidence', () => {
  ...
  it('sets minConfidence within valid range', () => {
    ...
    expect(appStore.settings().minConfidence).toBe(0.70);
  });

  it('clamps to minimum 0.30', () => {
    setMinConfidence(0.10);
    expect(appStore.settings().minConfidence).toBe(0.30);
  });

  it('clamps to maximum 0.90', () => {
    setMinConfidence(1.00);
    expect(appStore.settings().minConfidence).toBe(0.90);
  });
});
