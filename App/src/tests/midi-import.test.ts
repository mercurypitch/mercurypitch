...
function buildMidiFormat1(
  tracks: Array<Array<{ tick: number; type: 'on' | 'off'; pitch: number; vel: number }>>,
  ticksPerBeat = 480
): Uint8Array {
  const velocity = 80;
  const header = new Uint8Array(14);
  ...
  header[10] = 0; header[11] = tracks.length;                               // ntracks
  header[12] = (ticksPerBeat >> 8) & 0xff;
  header[13] = ticksPerBeat & 0xff;

  const trackChunks: Uint8Array[] = [];
  for (const trackEvents of tracks) {
    const absEvents = trackEvents.map(e => ({ tick: e.tick, type: e.type as 'on' | 'off', pitch: e.pitch, vel: e.vel ?? velocity }));
    absEvents.sort((a, b) => a.tick - b.tick);
...
function buildMidiWithTempo(tempo: number, notes: Array<{ pitch: number; startTick: number; endTick: number }>): Uint8Array {
  ...
  tempoChunk[0] = 0x4D; tempoChunk[1] = 0x54; tempoChunk[2] = 0x72; tempoChunk[3] = 0x6B;
  ...
  noteChunk[0] = 0x4D; noteChunk[1] = 0x54; noteChunk[2] = 0x72; noteChunk[3] = 0x6B;
  ...
  header[10] = 0; header[11] = 2;
  header[12] = 0; header[13] = 96; // 480 TPB

  const midi = new Uint8Array(header.length + tempoChunk.length + noteChunk.length);
  midi.set(header, 0);
