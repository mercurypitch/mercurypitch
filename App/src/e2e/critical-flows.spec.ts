...
test.describe('Critical Flows — GH #121', () => {
  ...
  test.describe('Playback Controls', () => {
    ...
    test('Play → Pause → Resume cycle', async ({ page }) => {
      ...
      await expect(page.locator('.stop-btn').first()).toBeVisible({ timeout: 3000 });
    ...
    test('Practice mode buttons (Once / Repeat / Practice) switch modes', async ({ page }) => {
  ...
  test.describe('Piano Roll — Note Entry', () => {
    ...
    test('piano roll toolbar is fully visible', async ({ page }) => {
      ...
      // Place tool button
      await expect(toolbar.locator('.roll-tool-btn[data-tool="place"]')).toBeVisible();
    ...
    test('instrument selector changes instrument', async ({ page }) => {
      ...
      await expect(instrumentSelect.locator('option[value="piano"]')).toBeAttached();
      await expect(instrumentSelect.locator('option[value="sine"]')).toBeAttached();
  ...
  test.describe('Presets', () => {
    ...
    test('preset dropdown shows saved preset option', async ({ page }) => {
      ...
      await expect(page.locator(`#preset-datalist option[value="${name}"]`)).toBeAttached();
    });
  ...
  test.describe('Practice Mode', () => {
    ...
    test('Practice mode cycles input accepts valid values', async ({ page }) => {
      // Switch to Practice mode
    ...
    test('Practice sub-mode select exists in practice mode', async ({ page }) => {
      await page.locator('#btn-practice').click();
      await page.waitForTimeout(300);

      const subModeSelect = page.locator('#practice-sub-mode');
      if (await subModeSelect.count() > 0) {
        await expect(subModeSelect.locator('option[value="all"]')).toBeAttached();
        await expect(subModeSelect.locator('option[value="random"]')).toBeAttached();
        await expect(subModeSelect.locator('option[value="focus"]')).toBeAttached();
        await expect(subModeSelect.locator('option[value="reverse"]')).toBeAttached();
  ...
  test.describe('Settings Persistence', () => {
    ...
    test('Sensitivity preset changes affect sensitivity value', async ({ page }) => {
      await page.locator('#tab-settings').click();
      await page.waitForTimeout(3000);

      const sensitivitySlider = page.locator('#sensitivity');
      if (await sensitivitySlider.count() > 0 && await sensitivitySlider.isVisible()) {
  ...
  test.describe('Tab Navigation', () => {
    ...
    test('settings panel renders correctly', async ({ page }) => {
      ...
      // GitHub link
      await expect(page.locator('.about-link')).toHaveAttribute('href', /github\.com/);
    });
  ...
  test.describe('Practice Page Improvements (GH #138)', () => {
    ...
    test('Focus Mode play/pause controls work', async ({ page }) => {
      ...
      await expect(page.locator('.focus-play[title="Pause"]')).toBeVisible({ timeout: 3000 });
      ...
      // Should show resume
      await expect(page.locator('.focus-play[title="Continue"]')).toBeVisible({ timeout: 3000 });
