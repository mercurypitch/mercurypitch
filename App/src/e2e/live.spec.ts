...
test('live site settings', async ({ page }) => {
  ...
  console.log('Initial tabs:', JSON.stringify(await page.evaluate(() => 
    Array.from(document.querySelectorAll('[id^="tab-"]')).map(t => ({ id: t.id, class: t.className }))
  )));
  
  await page.locator('#tab-settings').click();
  await page.waitForTimeout(3000);
  
  console.log('After clicking Settings:', JSON.stringify(await page.evaluate(() => 
    Array.from(document.querySelectorAll('[id^="tab-"]')).map(t => ({ id: t.id, class: t.className }))
  )));
  
  console.log('Settings panel count:', await page.locator('#settings-panel').count());
