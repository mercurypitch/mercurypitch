...
async function handleForgotPassword(
  ...
  if (emailRl.allowed) {
    ...
    if (user) {
      await sendPasswordResetEmail(request, env, user)
    }
  }
...
async function handleResetPassword(
  ...
  await env.DB.prepare('DELETE FROM passwordResets WHERE tokenHash = ?')
    .bind(tokenHash)
    .run()
...
export async function handleAuth(
  ...
  switch (route) {
    ...
    case 'forgot-password':
      return handleForgotPassword(request, body, env, respond)
    case 'reset-password':
