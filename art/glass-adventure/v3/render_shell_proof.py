"""Render finalized Meshy shells with every exploded fragment kept above the review floor."""
import argparse
import json
...
for shard in shards:
    ...
    shard.location += Vector((centre.x*.4, centre.y*.4, (centre.z-height*.45)*.18 + height*.12))
camera.data.ortho_scale *= 1.25
camera.location.z += height*.06
