"""Evaluate a smaller coupe LOD from the validated clean50k donor-derived mesh."""
...
finish.cutter.select_only([obj]);modifier=obj.modifiers.new('review_lod','DECIMATE');modifier.ratio=min(1,50000/len(obj.data.polygons));modifier.use_collapse_triangulate=True;bpy.ops.object.modifier_apply(modifier=modifier.name)
...
report={'operation':'one conservative 50k triangle LOD from the cleaned actual donor','targetTriangles':50000,'triangles':len(bm.faces),'selfIntersectionPairs':len(pairs),'badEdges':sum(not e.is_manifold for e in bm.edges),'badVertices':sum(not v.is_manifold for v in bm.verts),'nonContiguousEdges':sum(not e.is_contiguous for e in bm.edges),'signedVolume':bm.calc_volume(signed=True),'sampledMaxDistanceToCleanMetres':max(forward),'sampledMaxDistanceFromCleanMetres':max(reverse)}
bm.to_mesh(obj.data);bm.free();finish.cutter.planar_uv(obj);(directory/'coupe-lod-50k.json').write_text(json.dumps(report,indent=2)+'\n');bpy.ops.wm.save_as_mainfile(filepath=str(directory/'coupe-lod-50k.blend'));print('CLEAN_SURFACE_LOD='+json.dumps(report))
