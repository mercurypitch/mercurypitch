...
bpy.ops.wm.open_mainfile(filepath=str(directory/'prepared.blend'));obj=next(o for o in bpy.context.scene.objects if o.get('role')=='intact')
...
for iteration in range(48):
    ...
    # Only diagnosed patches and their immediate blending ring move. The 3mm
...
(directory/'contact-relax-3mm-trial.json').write_text(json.dumps(report,indent=2)+'\n');bpy.ops.wm.save_as_mainfile(filepath=str(directory/'contact-relax-3mm-trial.blend'));print('CONTACT_RELAX_TRIAL='+json.dumps(report))
