...
# Keep the studio floor below every expanded fragment so the foot remains visible.
lowest = min((shard.matrix_world @ vertex.co).z for shard in shards for vertex in shard.data.vertices)
floor.location.z = min(floor.location.z, lowest - height * .02)
