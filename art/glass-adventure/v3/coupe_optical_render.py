...
args = parser.parse_args(['--name','aurora-coupe'])
directory = HERE / 'donors' / args.name
manifest = json.loads((directory / 'optical-prepared.json').read_text())
bpy.ops.wm.open_mainfile(filepath=str(directory / 'optical-prepared.blend'))
...
scene.render.filepath = str(directory / 'optical-preview.png')
