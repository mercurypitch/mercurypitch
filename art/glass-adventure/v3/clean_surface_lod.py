"""Measure one conservative LOD of the repaired donor, preserving a clean fallback."""
...
finish.cutter.select_only([obj]);modifier=obj.modifiers.new('review_lod','DECIMATE');modifier.ratio=min(1,200000/len(obj.data.polygons));modifier.use_collapse_triangulate=True;bpy.ops.object.modifier_apply(modifier=modifier.name)
...
report={'operation':'one conservative 200k triangle LOD from the cleaned actual donor','targetTriangles':200000,'triangles':len(bm.faces),'selfIntersectionPairs':len(pairs),'badEdges':sum(not e.is_manifold for e in bm.edges),'badVertices':sum(not v.is_manifold for v in bm.verts),'nonContiguousEdges':sum(not e.is_contiguous for e in bm.edges),'signedVolume':bm.calc_volume(signed=True),'sampledMaxDistanceToCleanMetres':max(forward),'sampledMaxDistanceFromCleanMetres':max(reverse)}
bm.to_mesh(obj.data);bm.free();finish.cutter.planar_uv(obj);(directory/'clean-surface-lod.json').write_text(json.dumps(report,indent=2)+'\n');bpy.ops.wm.save_as_mainfile(filepath=str(directory/'clean-surface-lod.blend'));print('CLEAN_SURFACE_LOD='+json.dumps(report))
