// Resonance Veil inspection — actual renderer at fixed poses, not a gameplay acceptance test.
...
const moduleName = 'enclosed-chamber.ts'
const exportName = 'GLASS_ENCLOSED_CHAMBER'
  ...
  for (const viewport of [
    ...
    const summary = await page.evaluate(
      async ({ root, moduleName, exportName, mapping }) => {
        ...
        renderer.render(game.snapshot(), 0)
    ...
    console.log(JSON.stringify({ viewport, summary }))
    if (summary.errors.length) throw new Error(summary.errors.join('\n'))
    const poses = JSON.parse(
      await readFile(new URL('./poses.json', import.meta.url), 'utf8'),
    )
    for (const pose of poses) {
      ...
      records.push({ viewport, pose, file: filename, ...data })
