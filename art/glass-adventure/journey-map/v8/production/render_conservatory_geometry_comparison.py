"""Render CPU-only clay comparisons of the dense, V6, and V8 conservatory meshes."""
...
ART = HERE.parent
FRONT = ART / "proofs" / "conservatory-geometry-front-v8.png"
ANGLE = ART / "proofs" / "conservatory-geometry-angle-v8.png"
MANIFEST = ART / "proofs" / "conservatory-geometry-comparison-v8.json"
REMESH_RECEIPT = ART / "meshy" / "conservatory-remesh-110k-receipt.json"
TARGET_DIMENSIONS = (2.30, 2.65, 2.30)
ASSETS = (
    (
        "archived-pre-remesh",
        ART / "meshy" / "raw" / "conservatory-pre-remesh-v6.glb",
        -3.0,
        (0.74, 0.70, 0.62, 1.0),
    ),
    (
        "v6-textured-final-clay",
        ART.parent / "v6" / "meshy" / "conservatory-final.glb",
        0.0,
        (0.78, 0.62, 0.42, 1.0),
    ),
    (
        "v8-remesh-110k",
        ART / "meshy" / "conservatory-remesh-110k.glb",
        3.0,
        (0.52, 0.72, 0.64, 1.0),
    ),
)
...
def import_normalized(
    ...
    for index, obj in enumerate(meshes):
        ...
        for vertex in obj.data.vertices:
            vertex.co.x = (vertex.co.x - center_x) * scales.x + offset_x
    ...
    return {
        ...
        "displayDimensionsGlTfYUpMetres": list(TARGET_DIMENSIONS),
        "displayX": offset_x,
    }


def main() -> None:
    receipt = json.loads(REMESH_RECEIPT.read_text())
    remesh_path = ASSETS[2][1]
    if (
        receipt.get("state") != "archived"
        or receipt.get("file", {}).get("sha256") != digest(remesh_path)
    ):
        raise ValueError("V8 remesh does not match its archived receipt")
    ...
    bpy.ops.mesh.primitive_plane_add(size=22.0, location=(0.0, 0.0, -0.015))
    ground = bpy.context.object
    ground.name = "proof ground"
    ground.data.materials.append(material("proof ground", (0.11, 0.14, 0.18, 1.0)))
    add_area("warm key", (-5.0, -5.5, 7.0), 1450.0, 4.5, (1.0, 0.78, 0.58), (0.0, 0.0, 1.35))
    add_area("cool fill", (5.0, -2.5, 6.0), 1050.0, 4.0, (0.68, 0.84, 1.0), (0.0, 0.0, 1.4))
    add_area("rim", (0.0, 4.0, 6.5), 1250.0, 3.5, (0.70, 1.0, 0.86), (0.0, 0.0, 1.5))
    ...
    target = Vector((0.0, 0.0, 1.38))
    FRONT.parent.mkdir(parents=True, exist_ok=True)
    camera.location = (0.0, -14.0, 3.6)
    point_camera(camera, target)
    render(scene, camera, FRONT)
    camera.location = (5.6, -13.4, 4.4)
    point_camera(camera, target)
    ...
    result = {
        ...
        "order": ["archived pre-remesh", "V6 textured final shown as clay", "V8 110k remesh"],
        "assets": rows,
        "proofs": [
        ...
        "command": (
            ...
            "art/glass-adventure/journey-map/v8/production/render_conservatory_geometry_comparison.py"
        ),
    }
    MANIFEST.write_text(json.dumps(result, indent=2) + "\n")
    print("CONSERVATORY_GEOMETRY_PROOF=" + json.dumps(result), flush=True)
