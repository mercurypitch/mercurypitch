...
if source_hash != accepted["files"][str(source.relative_to(ROOT))]["sha256"]:
    raise ValueError("Reviewed baked source changed; inspect it before accepting new tangents.")
...
with tempfile.TemporaryDirectory(prefix="camellia-tangents-", dir=output.parent) as temporary:
    candidate = Path(temporary) / output.name
    repair.write(candidate, doc, binary)
    output_hash = hashlib.sha256(candidate.read_bytes()).hexdigest()
    if output_hash != accepted["files"][str(output.relative_to(ROOT))]["sha256"]:
        raise ValueError("Tangent repair drifted; the accepted export was preserved.")
    os.replace(candidate, output)
(ROOT / "proofs/tangent-repair.json").write_text(json.dumps({
    "sourceSha256": source_hash,
    "outputSha256": output_hash,
    "scriptSha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
    "attributeHelperSha256": hashlib.sha256(Path(repair.__file__).read_bytes()).hexdigest(),
