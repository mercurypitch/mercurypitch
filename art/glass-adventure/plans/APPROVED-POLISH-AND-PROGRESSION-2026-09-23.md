# Glass adventure — approved polish and progression delivery

Owner authorization: 23 September 2026. Complete roadmap items 1–8, choosing
small design details autonomously for the museum's calm, whimsical character.
Item 5 is included as a configurable melody implementation. Stop before item 9
(more map stages and levels) for owner testing. Items 10 (further mechanics and
rivals) and 11 (physical-device release acceptance/publication) remain later.
This supersedes older proposal-only labels for the work explicitly listed here.

Branch: `feat/glass-museum-level-one`; existing draft PR #807. Preserve stages in
separate commits and push. No merge or production release. Root plans, integrates
and reviews; GPT-5.6 Sol implements bounded independent slices. Work is complete
only with implementation, relevant tests, actual rendered evidence where needed,
saved source files, documentation and a passing PR gate. No placeholder controls,
unfinished code paths or rejected visual assets presented as completed work.

## Durable execution rules

- Read this file first after compaction, then the current checkpoint in TASKS.md
  under `<user-dotfiles>/personal/besidecue/glass-adventure/`.
- Update each stage with actual files, commands/results, next action and commit.
  Distinguish implementation complete from owner/device acceptance.
- Preserve original Meshy inputs/outputs, receipts, packed Blender projects and
  accepted/rejected comparisons in repository LFS. Mirror planning files into
  dotfiles; never stage another agent's unrelated dotfiles changes.
- Meshy credits and prepared-guide uploads are authorized. Prefer existing
  accepted sources; record new task IDs before polling and never duplicate a
  known paid submission. Keep credentials and signed URLs out of artifacts.
- Retain accepted fog, movement/jump reach and authored save identities. Keep
  owner preview free of hot-reload interruptions. Any server has a bounded
  lifetime and verified restart instructions.
- Use actual source and game comparisons, not structural validation alone, to
  accept art. Preserve detail using deliberate topology and isolated bakes.

## Decisions selected for this batch

- Fit new shallow Frost/Glide sources to existing landing bounds while preserving
  decorative proportions and visible relief. Collision is a separate proxy.
- Coins remain finite exploration discoveries; no shop, grind or paid economy.
- First Light stays an ungraded prologue. Existing full learning galleries gain
  authored first/two/three-star profiles. First clear is easy; either harder tier
  can be attempted afterward. Difficulty does not force a wider vocal range.
- Keep legacy accuracy results as portrait performance, never relabel them as
  harder difficulty clears. Preserve historical Cloudway eligibility.
- A level's tier requires every required exhibit and its exit in one compatible
  attempt. Optional discoveries and portraits survive replay/profile changes.
- Longer holds belong at selected mastery exhibits, not every vase. Melody,
  timing and tolerance are authored configuration, not buried constants.
...
live. First parallel wave: Frost/Glide derivative production, camera comfort,
and melody compiler/judge. Root implements replay/collection architecture and
reviews integration. No stage above has yet been declared complete.
