#!/usr/bin/env python3
"""Export and inventory the source-full-detail Rose crackle runtime candidate."""
...
BUILD = load_module(
    "cloudway_prepare_rose_runtime_candidate_delivery",
...
ASSET = BUILD.ASSET
RUNTIME_ID = "cloudway-lab-rose-crackle-v1"
...
def runtime_bindings() -> dict[str, dict[str, str]]:
    result = {
        ...
        "RoseRuntimeIvoryInlays": {
            "kind": "opaque",
            "material": VISUALS.IVORY_MATERIAL,
        },
    ...
    for index in range(18):
        result[f"RoseRuntimeShardGlass_{index:03d}"] = {
            "kind": "glass",
            "material": VISUALS.GLASS_MATERIAL,
        }
        result[f"RoseRuntimeShardSurfaceDetail_{index:03d}"] = {
            "kind": "opaque",
            "material": VISUALS.DETAIL_MATERIAL,
        }
    return result
...
def main() -> None:
    ...
    expected_collider = {
        "shape": "box",
        "width": BUILD.CONTACT_EXTENT,
        "depth": BUILD.CONTACT_EXTENT,
        "height": BUILD.COLLIDER_HEIGHT,
        "topY": 0,
        "center": [0, -BUILD.COLLIDER_HEIGHT * 0.5, 0],
    }
    require(
