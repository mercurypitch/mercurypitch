#!/usr/bin/env python3
"""Fresh-open audit the Rose hybrid hierarchy, geometry, and metadata contract."""
...
BUILD = load_module(
    "cloudway_prepare_rose_runtime_candidate_audit",
    HERE / "prepare_rose_crackle_runtime_candidate.py",
)
...
def audit_contact(contact: bpy.types.Object) -> dict[str, Any]:
    ...
    require(coordinates == expected, "Contact rectangle is not exact 1.64m square at Z=0")
...
def audit_shard(root: bpy.types.Object, expected_name: str) -> dict[str, Any]:
    ...
    cap_maximum = float(maximum.z + root.location.z)
    require(
        cap_maximum <= -0.0274,
        f"{root.name} evaluated glass cap lost its inward relief clearance",
    )
    ...
    return {
        ...
        "evaluatedCapMaximumWorldZMetres": round(cap_maximum, 9),
        "topology": topology,
...
def main() -> None:
    ...
    require(
        intact_materials == {VISUALS.GLASS_MATERIAL, VISUALS.DETAIL_MATERIAL},
        "Intact material bindings changed",
    )
    shell = only_object("RoseRuntimeCrystalShell")
    detail = only_object("RoseRuntimeProviderIntactExterior")
    corner_detail = only_object("RoseRuntimeSourceCornerFiligree")
    ...
    cap_maximum = max(row["evaluatedCapMaximumWorldZMetres"] for row in shard_audits)
    relief_minimum = (
        report["contact"]["sourceRayEvidence"]
        ["topOffsetFromLandingPlaneMillimetres"]["min"]
        / 1000.0
    )
    require(
        relief_minimum - cap_maximum >= 0.0275,
        "Evaluated shard glass is too close to the ray-audited provider relief",
    )
    image_failures = []
    ...
    audit = {
        "schema": "cloudway-rose-crackle-runtime-hybrid-audit/v1",
        "assetId": BUILD.ASSET,
        "status": "fresh-open candidate audit passed; visual acceptance and GLB export remain pending",
        ...
        "checks": [
            ...
            "evaluated closed-glass caps remain at least 27.5 mm inward of ray-audited top relief",
            ...
            "explicit glass, internal detail, framework, corner detail, and ivory material bindings",
            "exact 1.64m non-rendered contact rectangle with no ornament",
