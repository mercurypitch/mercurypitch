...
describe('initDefaultOGTags', () => {
  ...
  it('sets og:url to production URL', () => {
    initDefaultOGTags()
    expect(getMetaContent('property', 'og:url')).toBe('https://mercurypitch.com/')
  })

  it('sets og:image to favicon', () => {
    initDefaultOGTags()
    expect(getMetaContent('property', 'og:image')).toBe('https://mercurypitch.com/favicon.png')
  })
  ...
  it('sets twitter:image matching og:image', () => {
    initDefaultOGTags()
    expect(getMetaContent('name', 'twitter:image')).toBe('https://mercurypitch.com/favicon.png')
  })
...
describe('setMelodyOGTags', () => {
  ...
  it('updates og:title with melody info and key', () => {
    ...
    expect(getMetaContent('property', 'og:title')).toBe('Melody in C shared on PitchPerfect')
  })

  it('updates og:title without key', () => {
    setMelodyOGTags({ noteCount: 10 })
    expect(getMetaContent('property', 'og:title')).toBe('Melody shared on PitchPerfect')
  })
  ...
  it('updates og:url to current window location', () => {
    ...
    expect(getMetaContent('property', 'og:url')).toBe('https://mercurypitch.com/?n=m60s0d2')
    vi.unstubAllGlobals()
  })

  it('updates twitter:title to match melody title', () => {
    setMelodyOGTags({ noteCount: 10, key: 'G' })
    expect(getMetaContent('name', 'twitter:title')).toBe('Melody in G shared on PitchPerfect')
  })
...
describe('resetToDefaultOGTags', () => {
  ...
  it('reverts og:description to default', () => {
    ...
    resetToDefaultOGTags()
    expect(getMetaContent('property', 'og:description')).toContain('pitch')
    expect(getMetaContent('property', 'og:description')).not.toContain('15-note')
  })
  ...
  it('reverts twitter:description to default', () => {
    ...
    resetToDefaultOGTags()
    expect(getMetaContent('name', 'twitter:description')).toContain('pitch')
    expect(getMetaContent('name', 'twitter:description')).not.toContain('5-note')
  })
})
