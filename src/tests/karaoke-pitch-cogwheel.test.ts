...
// docs/specs/karaoke-pitch-cogwheel.ears.md (REQ-KPC-001..006).
//
...
/**
 * Edit-notes action availability -- suppressed on performance stage.
 * Prevents a dead-state where editMode is on but neither the panel
 * nor the toolbar is visible.
 */
const isCanEditEnabled = (
  preset: 'studio' | 'performance',
  hasEditableNotes: boolean,
): boolean => {
  return preset !== 'performance' && hasEditableNotes
}
...
describe('karaoke-pitch-cogwheel gates (REQ-KPC-*)', () => {
  ...
  // REQ-KPC-006: canEdit suppressed on performance stage
  it('KPC-5: canEdit is false on performance preset even with editable notes', () => {
    expect(isCanEditEnabled('performance', true)).toBe(false)
    expect(isCanEditEnabled('performance', false)).toBe(false)
  })

  it('KPC-5: canEdit reflects editable notes on studio preset', () => {
    expect(isCanEditEnabled('studio', true)).toBe(true)
    expect(isCanEditEnabled('studio', false)).toBe(false)
  })
})
