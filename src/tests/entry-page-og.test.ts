...
describe('the jam room invite link', () => {
  ...
  it('targets a path vite maps to jam.html', () => {
    // vite.config.ts builds its rewrite table from this record, so the record
    // is what has to be right — see src/seo/entry-pages.ts.
    const entry = ENTRY_PAGES.find((page) => page.slug === 'jam')
    expect(entry?.paths).toEqual(['/jam', '/jam-rooms'])
    expect(entry?.bootHash).toBe('#/jam')
  })
})
...
describe('the ear lab entry', () => {
  ...
  it('targets a path vite maps to ear-lab.html', () => {
    const entry = ENTRY_PAGES.find((page) => page.slug === 'ear-lab')
    expect(entry?.paths).toEqual(['/ear-lab'])
    expect(entry?.bootHash).toBe('#/ear-lab')
  })
