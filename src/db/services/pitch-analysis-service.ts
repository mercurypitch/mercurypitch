...
export async function saveOfflineAnalysis(
  ...
  try {
    ...
    const repo = db.getRepository<OfflinePitchAnalysisRecord>('offlinePitchAnalysis')
    ...
    const created = await repo.create({
      ...
      segmentedNotesJson: JSON.stringify(segmentedNotes !== undefined ? segmentedNotes : null),
    })
    return created.id
  } catch {
    return null
...
export async function getOfflineAnalysis(
  fileHash: string,
): Promise<{ analysisResults: unknown; lrcLines?: unknown; segmentedNotes?: unknown } | null> {
  try {
    const db = await getDb()
    const repo = db.getRepository<OfflinePitchAnalysisRecord>('offlinePitchAnalysis')
    ...
    const segmentedNotes = segmentedNotesRaw !== null ? segmentedNotesRaw : undefined
...
export async function deleteOfflineAnalysis(fileHash: string): Promise<boolean> {
  try {
    const db = await getDb()
    const repo = db.getRepository<OfflinePitchAnalysisRecord>('offlinePitchAnalysis')
    const existing = await repo.findAll({
