// ============================================================
// Premium background catalog store — route-neutral access evidence
// ============================================================
//
  ...
  return createRoot((disposeRoot) => {
      ...
      try {
        ...
        if (result === null) {
          ...
          setState({
            ...
            error: 'Premium backgrounds are unavailable offline.',
            revision: previous.revision + 1,
          })
        } else {
          const previous = state()
        ...
        setState({
          ...
          error:
            error instanceof BackgroundRequestError
              ? error.message
              : 'Premium backgrounds could not be checked.',
          revision: previous.revision + 1,
        })
      } finally {
    ...
    const invalidate = (id?: BackgroundPerkId, status?: number) => {
      ...
      setState({
        ...
        loading: false,
        error: 'Background access changed. Choose another backdrop.',
        revision: previous.revision + 1,
      })
      scheduleRefresh()
    }
