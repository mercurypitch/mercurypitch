// ============================================================
// Vocal Analysis — Mock Session Data Generator
// Generates realistic demo sessions for the VocalAnalysis tab
// when no practice history exists.
// ============================================================
...
function makeMelodyItem(midi: number, duration: number, startBeat: number): MelodyItem {
  return {
    id: midi * 100 + startBeat,
    note: makeNote(midi),
    duration,
    startBeat,
    velocity: 100,
...
export function generateMockSessions(): SessionResult[] {
  ...
  return MELODIES.map((melody, sessionIdx) => {
    ...
    const noteResults: NoteResult[] = melody.notes.map((midi, noteIdx) => {
      ...
      const centsOff = (Math.sin(noteIdx * 1.7 + sessionIdx) * 25 +
        (Math.random() - 0.5) * 20)
      ...
      const clarity = Math.max(10, Math.min(98, baseClarity + (Math.random() - 0.5) * 15))
