...
export const VoiceTypeDetectorModal: Component<VoiceTypeDetectorModalProps> = (props) => {
  const { practiceEngine } = useEngines()
  const [step, setStep] = createSignal<'intro' | 'listening' | 'result'>('intro')
  const [progress, setProgress] = createSignal(0)

  const [detectedPreset, setDetectedPreset] = createSignal<VocalRangePreset | null>(null)
  ...
  const startListening = async (e?: Event) => {
    ...
    try {
      ...
      const loop = () => {
        ...
        const currentProgress = Math.min(100, (goodFrames / TARGET_FRAMES) * 100)
  ...
  const finishListening = () => {
    ...
    if (median <= 52) { preset = 'bass'; noteName = 'E3 or lower' }
    else if (median <= 56) { preset = 'baritone'; noteName = 'G3 area' }
    else if (median <= 60) { preset = 'tenor'; noteName = 'C4 area' }
    else if (median <= 64) { preset = 'alto'; noteName = 'E4 area' }
    else if (median <= 69) { preset = 'mezzo-soprano'; noteName = 'A4 area' }
    else { preset = 'soprano'; noteName = 'C5 or higher' }
  ...
  return (
    <div class={styles.modalOverlay} onClick={(e) => { e.stopPropagation(); props.onClose(); }}>
        ...
        <div class={styles.modalBody}>
          <Show when={step() === 'intro'}>
            <div class={styles.centerContainer}>
              ...
              <p>We'll determine your natural voice type. Take a deep breath and sing a comfortable, steady <strong>"Ah"</strong> in your normal speaking range.</p>
              <button type="button" class={styles.actionBtn} onClick={(e) => void startListening(e)}>Start</button>
            </div>
          </Show>
          ...
          <Show when={step() === 'result'}>
            <div class={styles.centerContainer}>
              ...
              <div class={styles.resultActions}>
                <button type="button" class={styles.secondaryBtn} onClick={(e) => void startListening(e)}>Try Again</button>
                <button type="button" class={styles.actionBtn} onClick={(e) => { e.stopPropagation(); handleApply(); }}>Use This Preset</button>
              </div>
            </div>
          </Show>
        </div>
