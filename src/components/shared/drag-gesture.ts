...
export function dragGesture(
  ...
  const activatePointer = (
    ...
    // pointerdown's preventDefault suppresses the browser's native focus, so
    // restore the native-range contract for sliders: click focuses, then the
    // keyboard adjusts. Non-focusable surfaces make this a no-op.
    if (options.slider !== undefined) element.focus({ preventScroll: true })
  ...
  const onPointerMove = (event: PointerEvent): void => {
    if (pendingPointerId === event.pointerId && pendingStartEvent !== null) {
      // No buttons means the press ended outside the element (we never saw
      // its pointerup because pending gestures hold no capture). Clear the
      // stale press instead of activating a drag from a hover: capturing a
      // hovering pointer silently no-ops, and the surface would otherwise
      // start "dragging" with nothing pressed.
      if (event.buttons === 0) {
        pendingPointerId = null
        pendingStartEvent = null
        return
