        ...
        <div class="sm-lyrics-gen-row sm-lyrics-gen-row--settings">
          ...
          >
            <input
              ...
              onChange={(e) => props.setLiveHighlight(e.currentTarget.checked)}
            />
          ...
          fallback={
            <>
              ...
              <Show when={props.lrcGenPass() === 'words'}>
                {' '}
                Line starts are frozen, so the cursor begins at the second word
                and single-word lines are skipped.
              </Show>
            </>
          }
        >
          Lines — place only the start of each line. Fetched lyrics usually have
          these already, so play through and re-tap only the lines that drift,
          then switch to Words. Map everything in one stream with All.
        </Show>
