...
//
// The icon is GuideVocalMic — a stage mic, violet when the guide is audible
// and red when it is not. It used to be a line-art call-mic with a slash,
// which is the same picture the room uses for YOUR microphone; two controls
// one row apart meaning opposite things by the same glyph.
...
export const JamGuideVocal: Component<JamGuideVocalProps> = (props) => {
  ...
  return (
    ...
    >
      {/* Smaller than the karaoke stage's: this capsule is 36px in a row of
          room controls, not the star of a full-screen performance view. */}
      <GuideVocalMic muted={muted()} size={24} />
    </PillControl>
  )
}
