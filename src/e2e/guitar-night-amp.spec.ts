...
const STORAGE_V1 = 'guitar-night-amp-settings-v1'
const STORAGE_V2 = 'guitar-night-amp-settings-v2'
...
async function guardPassiveAudio(page: Page): Promise<string[]> {
...
async function audioActivity(page: Page): Promise<AudioActivity> {
  return page.evaluate(
    () => (window as unknown as { __ampActivity: AudioActivity }).__ampActivity,
  )
}

async function openSongAmp(page: Page): Promise<Locator> {
  await page
...
async function reloadSongAmp(page: Page): Promise<Locator> {
  await page.reload({ waitUntil: 'domcontentloaded' })
  await page.getByRole('button', { name: 'Enter room', exact: true }).click()
  return openSongAmp(page)
}

async function storedAmp(page: Page): Promise<Record<string, unknown> | null> {
  return page.evaluate(
