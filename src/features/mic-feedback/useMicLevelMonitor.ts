import type { Accessor } from 'solid-js'
import { createEffect, createSignal, onCleanup } from 'solid-js'

interface MicLevelMonitorOptions {
  /** Is the microphone currently on. */
  micActive: () => boolean
  /** Current input level as RMS amplitude (0–1). */
  getLevel: () => number
  /** The detector's minimum-amplitude threshold below which pitch is ignored. */
  minAmplitude: () => number
}

interface MicLevelMonitor {
  /** Smoothed input level (0–1). */
  level: Accessor<number>
  /**
   * True when the mic is picking up sound that is audible but sits below the
   * detection threshold for a sustained moment — i.e. "we hear you, but it's
   * too quiet to read your pitch".
   */
  tooQuiet: Accessor<boolean>
}

// Below this RMS we treat the signal as effectively silent (ambient noise),
// so we don't nag when the user simply isn't singing.
const NOISE_FLOOR = 0.006
// Sustained low-but-audible frames before we surface the hint (~0.75s @ 60fps),
// so it doesn't flicker between syllables.
const QUIET_FRAMES_TO_WARN = 45

/**
 * Monitors the live mic input level (only while the mic is on) and derives a
 * debounced "too quiet to detect" state. Runs a single rAF loop that starts
 * when the mic turns on and is cancelled when it turns off.
 */
export function useMicLevelMonitor(
  opts: MicLevelMonitorOptions,
): MicLevelMonitor {
  const [level, setLevel] = createSignal(0)
  const [tooQuiet, setTooQuiet] = createSignal(false)

  createEffect(() => {
    if (!opts.micActive()) {
      setLevel(0)
      setTooQuiet(false)
      return
    }

    let raf = 0
    let quietFrames = 0
    let smoothed = 0

    const tick = () => {
      smoothed = smoothed * 0.8 + opts.getLevel() * 0.2
      setLevel(smoothed)

      const min = opts.minAmplitude()
      if (smoothed >= min) {
        // Loud enough to be detected.
        quietFrames = 0
        setTooQuiet(false)
      } else if (smoothed > NOISE_FLOOR) {
        // Audible but below the detection threshold.
        quietFrames += 1
        if (quietFrames >= QUIET_FRAMES_TO_WARN) setTooQuiet(true)
      } else {
        // Effectively silent — not singing.
        quietFrames = 0
        setTooQuiet(false)
      }

      raf = requestAnimationFrame(tick)
    }

    raf = requestAnimationFrame(tick)
    onCleanup(() => cancelAnimationFrame(raf))
  })

  return { level, tooQuiet }
}
