...
const CARDS: ExerciseCardDef[] = [
  ...
  },
  {
    ...
    title: 'Pitch Hold',
    description: 'Keep your pitch inside a shrinking target zone. How long can you stay locked in?',
...
function gradeLabel(score: number): JSX.Element {
  if (score >= 90) return <><IconStar size={12} /> Elite</>
  if (score >= 80) return <><IconDiamond size={12} /> Great</>
  if (score >= 65) return <><IconCircleFill size={12} /> Good</>
  if (score >= 50) return <><IconCircleEmpty size={12} /> Novice</>
  return <></>
}
