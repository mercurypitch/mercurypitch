...
// "play" matches, "play that funky music" does not. NUMERIC homophone repair
// (e.g. "for" as 4) is intentionally NOT attempted; recognizers emit digits
// for numbers in command context, and guessing turns lyrics into numbers.
// Proper-noun substitutions are a different, safe class — see
// WORD_SUBSTITUTIONS.
...
/**
 * Whole-token repairs for words recognizers reliably replace with a proper
 * noun. Chrome hears "sing" and writes the surname "Singh" — which made
 * "Mercury Sing" almost impossible to trigger by voice, since the phrase
 * the grammar received never contained the word the user said.
 *
 * The bar for adding an entry: the LEFT side must be a word no command
 * grammar would ever want literally (a name, a brand misspelling), so the
 * rewrite cannot swallow a real utterance. Numbers stay out of here — see
 * the header.
 */
const WORD_SUBSTITUTIONS = new Map<string, string>(
  Object.entries({
    singh: 'sing',
    sings: 'sing',
    shazaam: 'shazam',
...
/**
 * Lowercase, strip diacritics and punctuation, collapse whitespace, then
 * apply the proper-noun repairs above. Dots survive only when they start a
 * decimal ("1.5", ".5") so numeric tokens keep their value while "play."
 * still reads as "play".
 */
export function normalizeUtterance(text: string): string {
  ...
  return cleaned
    .split(' ')
    .map((token) => WORD_SUBSTITUTIONS.get(token) ?? token)
    .join(' ')
}
