...
describe('a document nobody has touched yet', () => {
  function setActivation(hasBeenActive: boolean | undefined): void {
  ...
  it('does not start a session iOS would never feed', () => {
    setActivation(false)
    const h = harness({ visibleRespawn: true })

    h.listener.start()

    // The platform grants the recognizer, reports `start` and `audiostart`,
    // and then delivers no audio for the life of the session — no error, no
    // end. Measured on a device: every session started from the saved
    // preference during boot was deaf, and every session a person started by
    // tapping worked.
    expect(FakeRecognition.instances).toHaveLength(0)
    // A resting mic, not a fault. One touch anywhere is all it waits for.
    expect(h.last().state).toBe('dozing')
  })

  it('starts on the first touch, and only wants one', () => {
    setActivation(false)
    const h = harness({ visibleRespawn: true })
    h.listener.start()
  ...
  it('starts at once for a visitor who has already touched the page', () => {
    setActivation(true)
    const h = harness({ visibleRespawn: true })
  ...
  it('leaves desktop alone, where activation was never the price', () => {
    setActivation(false)
    const h = harness({ visibleRespawn: false })

    h.listener.start()

    // Waiting here would buy nothing and cost the visitor a click.
    expect(FakeRecognition.instances).toHaveLength(1)
  })

  it('starts anyway when the browser will not say', () => {
    setActivation(undefined)
    const h = harness({ visibleRespawn: true })

    h.listener.start()

    // A browser that does not report activation is one where this has never
    // been a problem. Guessing "not activated" there would silence it.
    expect(FakeRecognition.instances).toHaveLength(1)
  })
})
