...
export function createTransportVoiceCommands(
  ...
  const doPlay = (): VoiceCommandResult => {
    ...
    if (t === TAB_GUITAR && deps.handlers.guitar?.togglePlayback) {
      // With a play-state to read, the verb gains direction; without one
      // (older wiring) it stays the Space key's blind flip.
      const guitarPlaying = deps.handlers.guitar.isPlaying?.()
      if (guitarPlaying === true) return voiceFailure('Already playing')
      deps.handlers.guitar.togglePlayback()
      return guitarPlaying === undefined ? 'Toggle playback' : 'Play'
    }
  ...
  const doPause = (): VoiceCommandResult => {
    ...
    if (t === TAB_GUITAR && deps.handlers.guitar?.togglePlayback) {
      const guitarPlaying = deps.handlers.guitar.isPlaying?.()
      if (guitarPlaying === false) return voiceFailure('Nothing playing')
      deps.handlers.guitar.togglePlayback()
      return guitarPlaying === undefined ? 'Toggle playback' : 'Pause'
    }
    if (t === TAB_COMPOSE && deps.handlers.editor) {
  ...
  const doStop = (): VoiceCommandResult => {
    ...
    if (t === TAB_GUITAR && deps.handlers.guitar?.togglePlayback) {
      ...
      if (guitarPlaying === false) return voiceFailure('Nothing playing')
      deps.handlers.guitar.togglePlayback()
      return guitarPlaying === undefined ? 'Toggle playback' : 'Stop'
    }
    if (t === TAB_COMPOSE && deps.handlers.editor) {
      if (deps.handlers.editor.isPlaying()) {
        deps.handlers.editor.pause()
