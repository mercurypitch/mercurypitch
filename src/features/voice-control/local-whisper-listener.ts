...
/**
 * Mic holds are per-RUN, not per-listener: MicManager tracks consumers by
 * id in a set, so two runs sharing one id can cancel each other — a stale
 * start (engine switched while the first acquire was still queued) would
 * release the id the new run depends on, and voice control went silently
 * deaf. Module-level so ids stay unique across listener instances too
 * (whisper and moonshine are separate closures).
 */
let micConsumerSerial = 0
...
export function createLocalWhisperListener(
  ...
  let runGeneration = 0
  let micHeld = false
  /** The per-run consumer id the CURRENT run acquired with. */
  let micConsumerId: string | null = null
  ...
  const startAsync = async () => {
    ...
    try {
      ...
      micHeld = true
      micConsumerId = consumerId
      if (!started || generation !== runGeneration) {
        // Stale run: release OUR hold only — the per-run id cannot touch
        // the hold a newer run has meanwhile acquired.
        micHeld = false
        micConsumerId = null
      ...
      if (audioCtx.state === 'suspended') {
        // A persisted-on flag starts this at page load with NO user
        // gesture, and the autoplay policy may refuse the resume: all of
        // this wires up, the state says listening, and onaudioprocess
        // never fires. Retry on the first gesture; idempotent, and the
        // handlers leave with the run.
        const ctx = audioCtx
