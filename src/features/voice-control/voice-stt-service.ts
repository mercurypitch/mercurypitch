...
export class VoiceSttService {
  ...
  async init(): Promise<void> {
    ...
    // 'error' re-posts load too: a failed download (model host hiccup, rate
    // limit without CORS headers, offline) must not poison the page-lifetime
    // instance — the worker resets itself on failure precisely so the next
    // attempt retries. Without this, a retry waited on a status message
    ...
    return new Promise((resolve, reject) => {
      ...
      const watchdog = setInterval(() => {
        if (Date.now() - startedAt > LOAD_TIMEOUT_MS) {
          cleanup()
          // Mark the protocol failed so the NEXT init re-posts load instead
          // of waiting behind a state that will never change.
