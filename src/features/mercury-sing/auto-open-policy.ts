...
export interface AutoOpenPolicy {
  ...
  /**
   * Reporting is being suspended — the engine freezes ranking while the
   * singer SPEAKS, so the next report may arrive seconds late. Sustain is
   * measured evidence, and a frozen span was never measured: without this,
   * `nowMs - armedSinceMs` would credit the whole silence as held
   * threshold+margin and a single post-freeze tick could open the song.
   * Drops any arming in progress; the one-open latch stays. Idempotent.
   */
  interrupt(): AutoOpenSnapshot
}

/**
 * Provisional until field data lands (docs/plans/mercury-sing-field-test.md).
 *
 * The threshold was 0.95, chosen before anyone measured what the matcher
 * can actually produce. It could not be reached: a FLAWLESS excerpt of a
 * long song scored 0.91, so the band could never join in on its own. With
 * the matcher fixed a perfect excerpt scores 1.0 and a wrong song ~0.25,
 * which leaves room — but real singing is not flawless, and nobody has
 * measured where a real take lands yet. 0.80 is a deliberate
 * under-commitment: the margin over the runner-up and the sustain window
 * are doing the real work, and the field test will replace this guess with
 * a number.
 */
