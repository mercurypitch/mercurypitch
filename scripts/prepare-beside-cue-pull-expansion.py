...
def decode(path):
    ...
    process = subprocess.Popen(["ffmpeg", "-v", "error", "-threads", "1", "-i", str(path),
                                "-vf", "fps=24", "-f", "rawvideo", "-pix_fmt", "rgb24", "-"],
...
def main():
    ...
    manifest = {"revision": "pull-expansion-v1", "plateSha256": digest(plate_path), "pulls": {}}
    ...
    for index, pull_id in enumerate(args.pull or IDS):
        ...
        def rendered(source, exiting=False):
            for n, raw in enumerate(decode(source)):
                if exiting and n == 0:
                    ...
                    # Source-edge cropping must happen outside the ROOM, not
                    # at the smaller keyed plate's interior registration edge.
                    travel = max(0, offset[0] + 40)
                    shift = -travel * (min(1, max(0, (n - 48) / 47)) if exiting else max(0, 1 - n / 24))
                    moving_offset = (round(offset[0] + shift), offset[1])
                    yield composite(plate, register(key(raw, pull_id, exiting), scale, moving_offset))
            if exiting:
                # A settled empty-room tail gives the next beat a stable seam.
                for _ in range(6):
                    yield plate.convert("RGB")
        record = {"sources": {}, "registration": {"scale": scale, "offset": offset, "edgeTravelPx": max(0, offset[0] + 40)}, "matte": "pillow-beat-chroma-silhouette" if pull_id == "the-pillow" else "paper-chroma" if pull_id in ("the-tab", "the-bookmark") else "chroma-ratio", "frames": {}}
        for moment, source in [("present", present), ("recede", recede)]:
            ...
            if not args.stills_only:
                name = f"{'b03' if moment == 'present' else 'b05'}-{pull_id}-{moment}-v0_1.mp4"
